...
| HELIX-FR-002 | POST /api/v1/transformations/{id}/experiments (title, hypothesis, nextAction, optional cadence/evidenceOfSuccess/reviewAt) plus GET /api/v1/transformations/{id}/experiments/draft for reviewable AI/fallback experiment prefills with provenance, surfaced by the guided experiment form on TransformationDetailPage | ExperimentServiceTest, NoAiAssistantAdapterTest, TransformationDetailPage.test.tsx | ADR-001, ADR-003, ADR-004, ADR-008, ADR-016 |
...
| HELIX-FR-013 | POST /api/v1/wisdom/weekly-retrospective and GET /api/v1/wisdom/retrospectives snapshot history, surfaced in the Library weekly retrospective workspace with save confirmation and visible snapshot history | WeeklyRetrospectiveService test, web wisdom page test | ADR-004, ADR-010 |
...
| HELIX-FR-017 | POST /api/v1/memory/proposals and review actions with provenance and lifecycle history, including actionable source-validation errors for unresolved source records | MemoryProposalServiceTest, MemoryProposalControllerTest, MemoryPage.test.tsx | ADR-008, ADR-010 |
...
| HELIX-BR-002 | Evidence provenance payload/detail rendering and experiment-draft provenance (AI vs. fallback, provider/model) surfaced before save | EvidenceService test, TransformationDetailPage.test.tsx | ADR-009, ADR-016 |
