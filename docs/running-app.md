...
As of ADR-016, three features are AI-generated by default: the "Suggested Small Action" shown
after saving a reflection, the weekly retrospective narrative, and the optional "Draft this for
me" experiment proposal on a transformation's page. `HELIX_AI_PROVIDER=none` (or a provider
outage) falls back to generic templated text for all three rather than an equally-maintained
deterministic experience. Provenance (AI vs. fallback, and which provider/model) is shown in the
UI and stored with the suggestion/retrospective.
